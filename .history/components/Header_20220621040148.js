...
const Header = () => {
  return (
    <header className='grid grid-cols-3 shadow-md bg-white sticky top-0 z-50 items-center py-1 px-4 space-x-2'> 
      {/* Left */}
      <div className='relative h-14 md:h-16 cursor-pointer'>
        <Image 
      ...
      {/* Middle */}
      <div className='flex flex-grow border rounded-full py-1 space-x-2 items-center px-2 hover:ring-1 ring-gray-600'>
        <input 
          type="text" 
          placeholder='Search'
      ...
      <div className='flex justify-end space-x-5 text-gray-600 items-center'>
        <div className='flex space-x-3'>
          <button className='outline-none'><p className='hidden md:inline-flex cursor-pointer hover:underline md:font-bold lg:text-xl'>Become a host</p></button>
          <button className='outline-none'><GlobeAltIcon className='h-6 cursor-pointer'/></button>
        </div>
        <div className='flex border p-2 rounded-full'>
        <button className='outline-none'><ViewListIcon className='h-6 cursor-pointer'/></button>
        <button className='outline-none'><UserCircleIcon className='h-6 cursor-pointer'/></button>
        </div>
      </div>
    </header>
  )
